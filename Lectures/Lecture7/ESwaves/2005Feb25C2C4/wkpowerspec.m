function [power,freq,wavenumber] = wkpowerspec(VL1,B,trange, SCnum, probecomb) 
%
...
% EFW's internal burst mode data. By D. B. Graham.
%
...
%
% Example:  
% tint = [irf_time([2006 04 28 03 41 35.22]) irf_time([2006 04 28 03 41 35.75])];
% VL1 = c_caa_var_get('Data__C4_CP_EFW_L1_IB','caa','mat');
% B = c_caa_var_get('B_vec_xyz_gse__C4_CP_FGM_FULL','caa','mat');
...
if (thetatest > 0)
    c_eval('W1c = irf_wavelet_dg([time,E?]);',probe+1);
    c_eval('W2c = irf_wavelet_dg([time,E?]);',probe);  
else
  c_eval('W1c = irf_wavelet_dg([time,E?]);',probe);
  c_eval('W2c = irf_wavelet_dg([time,E?]);',probe+1);
end
...
for m = [1:1:N+1]
    ...
    Powerav(m,:) = nanmean(Power([posav(m)-cav/2+1:posav(m)+cav/2],:));
end
